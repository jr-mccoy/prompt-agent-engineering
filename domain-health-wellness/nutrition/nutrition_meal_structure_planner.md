...
> fasting, or exercise to "undo" food); a goal of more than about 1% of body weight a
> week, a large loss by a date, or an intake below commonly cited unsupervised floors
> (roughly 1,200 kcal/day for women, 1,500 for men); age under 18; pregnancy or
...
> for a medical condition. The redirect uses the same short, kind wording as
> `nutrition/nutrition_eating_pattern_audit.md`: what was noticed, a clinician and a
> registered dietitian as the next step, the crisis route if the person feels unsafe
> (in the US, call or text 988) — then stop.
...
1. **Readiness Profile** — nutrition guard must be CLEAR.
...
1. **Run the guard (QA-08).** Profile line CLEAR and no STRONG-GUARD signal in the
   conversation. Otherwise redirect and stop.
2. **Set the default plate (DS-26).** Hand-portion defaults, adjusted by appetite:
   ...
   General guidance for active adults is often expressed as roughly 1.2–1.6 g of protein
   per kg of body weight a day; portions are used here instead, so no weighing or body
   weight is needed.
...
- [ ] The guard ran first; any signal produced only a redirect.
...
Scenario: Leo (from the audit), 29, CLEAR, strength 18:30 Mon/Wed/Fri, tight budget,
