...
> Fill this during stages 1–3 (see [`../PIPELINE_OVERVIEW.md`](../PIPELINE_OVERVIEW.md) and [`../prompts/`](../prompts/)). It is the load-bearing decisions doc — the "why," not just the "what." Keep it honest: record rejected options and open questions. *(Self-contained copy of `authoring/system-patterns/templates/ARCHITECTURE_TEMPLATE.md` with factory marker footers added.)*
...
```
- Per-agent details → one [`AGENT_SPEC_TEMPLATE.md`](AGENT_SPEC_TEMPLATE.md) each.
- Per-tool details → one [`TOOL_SPEC_TEMPLATE.md`](TOOL_SPEC_TEMPLATE.md) each.
...
> The factory's `scripts/check_gate.py` reads these from the **emitted** `ARCHITECTURE.md`. Set them when you fill §2. Markers are HTML comments (invisible when rendered). Full contract: [`BUNDLE_MANIFEST_TEMPLATE.md`](BUNDLE_MANIFEST_TEMPLATE.md).
