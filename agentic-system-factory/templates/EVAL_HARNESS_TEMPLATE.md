...
> `scripts/check_gate.py --gate B` requires **both** markers below in the emitted `EVAL_HARNESS.md`. Missing either ⇒ Gate B FAIL (capability ≠ safety). The fenced example below is **inert** — the script ignores markers inside code fences; emit live markers in your file. Full contract: [`BUNDLE_MANIFEST_TEMPLATE.md`](BUNDLE_MANIFEST_TEMPLATE.md).
