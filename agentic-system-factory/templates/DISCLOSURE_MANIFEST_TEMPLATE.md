...
> `scripts/check_gate.py --gate C` requires all six dimension markers in the emitted `DISCLOSURE_MANIFEST.md` (plus a rollback path in `RUNBOOK.md` and a present `OBSERVABILITY.md`). The fenced example below is **inert** — the script ignores markers inside code fences; emit live markers in your file, one per dimension *actually completed*. Full contract: [`BUNDLE_MANIFEST_TEMPLATE.md`](BUNDLE_MANIFEST_TEMPLATE.md).
