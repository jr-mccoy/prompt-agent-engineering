...
> `scripts/check_gate.py --gate B` requires **both** markers below in the emitted `EVAL_HARNESS.md`. Missing either ⇒ Gate B FAIL (capability ≠ safety). The fenced example below is **inert** — the script ignores markers inside code fences; emit live markers in your file. Full contract: [`BUNDLE_MANIFEST_TEMPLATE.md`](../../templates/BUNDLE_MANIFEST_TEMPLATE.md).

```
<!-- GATE-B-CAPABILITY: present -->   ABC-valid: task-validity + outcome-validity + trivial-agent baseline
<!-- GATE-B-SAFETY: present -->       real-tool safety eval covering the 8 OpenAgentSafety categories
```
- Both are independent gates. A capable system with no safety marker does **not** pass Gate B.
