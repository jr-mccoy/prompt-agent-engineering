...
- **Coverage audit pass: fixes to Waves 1–5, a plain-language tag pass, and the deferred legal and children's-writing backlogs** ([`meta/COVERAGE_ROADMAP.md`](meta/COVERAGE_ROADMAP.md) §6, "Audit pass").
  - **New prompts (33):**
    ...
    - Legal EXPANSION_ROADMAP Phase 3: 10 discovery and litigation prompts plus `domain-legal/field_guide.md`.
    - Legal Phase 4: 10 specialized prompts, filed in existing folders.
    - Children's-writing EXPANSION_ROADMAP items 1–12.
  - **Tag pass:** 634 plain-language situation tags on 258 Wave 1–4 prompts, written from each prompt's content without seeing the regression queries.
    - The 8 prompts retagged in Wave 5 were skipped.
    - Legal, psy-ops, negotiation and written-advocacy get one tag each. The full set flipped original case 004's top hit on a 0.012 score margin.
  - **Routing:**
    - R@1 76.7 → 77.9%, R@5 84.9 → 86.0%, MRR 0.803 → 0.815, scope@1 83.6 → 84.5%.
    - Flat BM25 stays at 75.6% R@1, so BM25F leads on every metric.
    - Case 131 now passes. Cases 124, 132, 138 and 140 remain honest failures, and 145 still routes `weak`.
    - No original case changes its top hit, and no query, label, floor or threshold was changed.
...
- **Audit of coverage Waves 1–5:**
  - **Safety:**
    ...
    - The psy-ops youth prompt made more defensive after a child-safety review.
  - **Clinical:** the 32 Wave 3 `interp_*`/`specialty_*` prompts gained `related_prompts`, When to Use, Verification, False-Positive Prevention, a decision-support line and escalation blocks, plus about 40 accuracy corrections.
  - **Legal:** outdated premises in examples, a mandatory-reporting screen, scope guards, and 4 broken README links.
  - **Health and psychology:** 12 further safety and consistency fixes.
  - **Overlaps:** five now state the distinction, and the older chatbot prompts point to the handoff prompt.
  - **Docs:** stale counts and links in 10 READMEs, `ROUTING_REFERENCE.md`, `REPO_MAP.md` and `EXTERNAL_CONTRIBUTOR_GUIDE.md`.
  - **References:** 13 old-style `decision-making/` `related_prompts`.
  - **xref baseline:** shrunk by 45 entries, none added.
  - **ADR-0043:** a dated amendment records that the deferred regression cases shipped and pass ADR-0037.
- Hard-coded registry counts: live prompts 4528 → 4561, total records 5637 → 5670. Tombstones (53), moves (248) and merged-into (15) are unchanged.
