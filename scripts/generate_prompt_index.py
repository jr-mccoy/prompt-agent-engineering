#!/usr/bin/env python3
"""
Generate the machine-readable index of the repository's indexed Markdown artifacts.

Despite the historical `PROMPT_INDEX` filenames, this index is a **mixed
artifact population**: domain prompts, agentic resources (skills, agents,
commands, personas), and bundled component files that belong to a parent
resource. Its raw total is therefore not a prompt count.

Classified counts, each with a stated membership rule, live in
``meta/REPOSITORY_FACTS.json`` (generated by ``scripts/generate_repo_facts.py``).

Output metadata exposes canonical fields (``total_indexed_artifacts``,
``index_semantics``, ``schema_version``) alongside deprecated legacy aliases
(``total_prompts`` and friends) kept for backward compatibility.
"""
...
# Version of the emitted index metadata shape. Bump when the metadata contract
# changes; consumers can branch on it. `1` is the first version to carry
# canonical (non-prompt-specific) field names alongside the legacy aliases.
...
def generate_json_index(prompts: List[Dict[str, Any]], output_path: Path):
    ...
    index = {
        'metadata': {
            'generated': today,
            # --- Canonical, semantically accurate fields -------------------
            # This index covers every indexed Markdown artifact, not only
            # prompts: it also contains agentic resources (skills, agents,
            # commands, personas) and bundled component files belonging to a
            # parent resource. Use these fields, not the legacy aliases below.
            'schema_version': INDEX_SCHEMA_VERSION,
            'index_semantics': 'mixed_artifacts',
            'total_indexed_artifacts': len(prompts),
            'artifacts_with_frontmatter': sum(1 for p in prompts if p['has_frontmatter']),
            'artifacts_without_frontmatter': sum(1 for p in prompts if not p['has_frontmatter']),
            'domains': len(set(p['domain'] for p in prompts)),
            # For the classified breakdown (domain prompt artifacts, agentic
            # resources, bundled component files) see meta/REPOSITORY_FACTS.json.
            'canonical_facts': 'meta/REPOSITORY_FACTS.json',
            # --- Legacy aliases, retained for backward compatibility -------
            # DEPRECATED. `total_prompts` mirrors `total_indexed_artifacts`
            # and is NOT a count of prompts. The top-level `prompts` array is
            # likewise an array of mixed artifacts. Both are kept so existing
            # consumers keep working; new consumers should use the canonical
            # fields. A future PAE Registry will publish normalized kinds.
            'deprecated_aliases': {
    ...
    print(f"  Total indexed artifacts: {len(prompts)}")
...
def generate_markdown_index(prompts: List[Dict[str, Any]], output_path: Path):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        today = date.today().isoformat()
        f.write("# Comprehensive Indexed Artifact Catalog\n\n")
        f.write(
            "> **The filename `PROMPT_INDEX.md` is retained for compatibility.** "
            "This catalog lists every indexed Markdown artifact, and that is not "
            "the same thing as a list of prompts: alongside domain prompts it "
            "contains agentic resources (skills, agents, commands, personas) and "
            "bundled component files belonging to a parent resource "
            "(`references/`, `assets/`, `cards/` and similar).\n>\n"
            "> For classified counts with stated membership rules, see "
            "[`meta/REPOSITORY_FACTS.json`](meta/REPOSITORY_FACTS.json). Normalized "
            "resource kinds are planned for the PAE Registry and do not exist yet — "
            "see [`ROADMAP.md`](ROADMAP.md).\n\n"
        )
        f.write(f"**Generated:** {today}\n\n")
        f.write(f"**Total Indexed Artifacts:** {len(prompts)}\n\n")
        f.write(f"**Artifacts with Frontmatter:** {sum(1 for p in prompts if p['has_frontmatter'])}\n\n")
        f.write(f"**Artifacts without Frontmatter:** {sum(1 for p in prompts if not p['has_frontmatter'])}\n\n")
...
def generate_statistics(prompts: List[Dict[str, Any]]):
    ...
    print("\n" + "="*60)
    print("INDEXED ARTIFACT STATISTICS")
    print("="*60)

    print(f"\nTotal Indexed Artifacts: {len(prompts)}")
    print("  (mixed kinds: domain prompts, agentic resources, bundled components)")
    print(f"Artifacts with Frontmatter: {sum(1 for p in prompts if p['has_frontmatter'])}")
    print(f"Artifacts without Frontmatter: {sum(1 for p in prompts if not p['has_frontmatter'])}")
