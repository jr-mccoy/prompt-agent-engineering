...
Validation contract (``--check``) — a missing match is ALWAYS an error:
  ...
  * PROMPT_INDEX.json lacks its neutral total/semantics declaration,
    disagrees with `indexed_artifacts`, or lets a legacy alias drift -> fail
...
# PROMPT_INDEX.json must keep declaring that it is a mixed artifact population.
# Changing this is a Phase 2 decision, not an editorial one.
EXPECTED_INDEX_SEMANTICS = "mixed_artifacts"
...
def _render_headline(facts: dict[str, int]) -> tuple[list[str], str]:
    ...
    lines = [
        ...
        "|---|---|",
        f"| Indexed domain prompt artifacts | {facts['domain_prompt_artifacts']} |",
        ...
        "",
        "*Indexed domain prompt artifacts* is the current index's **structural**"
        " classification — Markdown files outside `domain-agentic-resources/` that"
        " are not bundled components and not slash commands. It is not yet a"
        " normalized resource-kind count; resource kinds arrive with the PAE"
        " Registry (see [`ROADMAP.md`](ROADMAP.md)).",
    ]
    return keys, "\n".join(lines)
...
def _render_counts(facts: dict[str, int]) -> tuple[list[str], str]:
    ...
    lines = [
        f"- **{facts['domain_prompt_artifacts']} indexed domain prompt artifacts** "
        f"across {facts['domain_directories']} `domain-*` directories. This is the "
        "index's structural classification, not a normalized resource-kind count.",
        ...
        f"{facts['domain_prompt_artifacts']} domain prompt artifacts with "
...
def check_index_metadata(facts: dict[str, int], errors: list[str]) -> None:
    """The legacy index must declare neutral semantics and agree with the facts.

    ``PROMPT_INDEX.json`` keeps prompt-specific field names for backward
    compatibility, so this guards the three ways that could mislead:
    a missing neutral declaration, a neutral total that disagrees with the
    canonical ``indexed_artifacts``, and a legacy alias drifting away from the
    value it aliases.
    """
    path = REPO_ROOT / "PROMPT_INDEX.json"
    if not path.exists():
        errors.append("PROMPT_INDEX.json: missing")
        return

    with path.open(encoding="utf-8") as fh:
        metadata = json.load(fh).get("metadata", {})

    for key in ("schema_version", "index_semantics", "total_indexed_artifacts"):
        if key not in metadata:
            errors.append(
                f"PROMPT_INDEX.json: metadata is missing the required neutral field "
                f"'{key}' — regenerate with scripts/generate_prompt_index.py"
            )

    semantics = metadata.get("index_semantics")
    if semantics is not None and semantics != EXPECTED_INDEX_SEMANTICS:
        errors.append(
            f"PROMPT_INDEX.json: index_semantics is {semantics!r}, expected "
            f"{EXPECTED_INDEX_SEMANTICS!r}. The index is still a mixed artifact "
            f"population; normalized kinds are Phase 2 work."
        )

    neutral = metadata.get("total_indexed_artifacts")
    if neutral is not None and neutral != facts["indexed_artifacts"]:
        errors.append(
            f"PROMPT_INDEX.json: total_indexed_artifacts={neutral} but canonical "
            f"indexed_artifacts={facts['indexed_artifacts']}"
        )

    # Legacy aliases are allowed to exist; they are not allowed to disagree.
    for legacy, canonical in (
        ...
        if legacy in metadata and canonical in metadata:
            if metadata[legacy] != metadata[canonical]:
                errors.append(
                    f"PROMPT_INDEX.json: legacy alias {legacy}={metadata[legacy]} has "
                    f"drifted from {canonical}={metadata[canonical]}"
                )
...
def main() -> None:
    ...
    if args.check:
        ...
        check_index_metadata(facts, errors)
