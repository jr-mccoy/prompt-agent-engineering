...
Run the Phase 3 mandatory perspective roster defined in [`BACKBONE.md`](BACKBONE.md): red team, steel-man, blind-spot scan, future-self, naive newcomer, and affected party. For designs, each lens must include its lens statement, its take on the design, and the load-bearing dimension it most strongly flags.

#### 3b. Propose scope-specific additions

Use the design/architecture candidate pool in [`BACKBONE.md`](BACKBONE.md) to propose 2–4 additional perspectives tailored to the user's design context. Confirm the additions with `AskUserQuestion`/`**GATE:**` and run only the perspectives the user picks.
