...
| **Example Output** | Yes | One coherent end-to-end scenario carried through every section of the stated Output Format (see *Density, not length* below) |
| **Techniques Used** | Yes | 3-5 techniques with explanations |

#### Density, not length

Example Output is judged by **coverage and coherence, not line count**. A Tier-1 example is *one*
scenario carried end to end: every section named in the Output Format appears, populated with
values that are consistent with each other, and later sections reference earlier ones rather than
restating them. A dense 40-line example of a decision matrix can meet this bar completely, while
a padded 150-line example of disconnected fragments does not.

The earlier "100+ lines" wording was a proxy for that property and measured it badly — it read as
a floor to hit rather than a standard to meet, and every prompt in `domain-AI-ML/` (326 files
with examples ranging from 13 to 81 lines, all single-scenario and coherent) failed it on a
technicality while satisfying its intent. Use the criterion below instead:

- **Complete** — every section of the stated Output Format is present.
- **Coherent** — one scenario throughout; the numbers, names, and findings are mutually consistent.
- **Connected** — later sections refer back to earlier ones, so the example demonstrates reasoning
  rather than formatting.
- **Concrete** — real-looking values, or explicit `[measure]` / `[verify]` markers where a value
  must not be invented. Placeholder markers are preferred over fabricated numbers.

Length follows from the Output Format's complexity. A prompt whose output is a nine-column threat
register will run long; one whose output is a canvas will not, and padding it would make it worse.
...
> **Optional domain-specific fields:** some domains extend this with an additional machine-readable block — e.g. `domain-reasoning-craft/` prompts carry a `reasoning:` block (styles, stakes, horizon, uncertainty, mode). These are optional repo-wide; if you contribute to such a domain, copy the block from an existing sibling so metadata stays consistent. See [`authoring/NEW_PROMPT_TEMPLATE.md`](authoring/NEW_PROMPT_TEMPLATE.md#optional-domain-specific-frontmatter).
...
[One coherent end-to-end scenario, carried through every section of the Output Format above, with findings that reference each other]
...
| Expected Output | Yes | |
| Example Output | Yes | One coherent scenario covering every Output Format section |
...
7. Expand the example output until one coherent scenario covers every section of the stated Output Format
...
3. Add a comprehensive example output — one coherent scenario covering every Output Format section
...
- [ ] **Delimited inputs:** If the prompt consumes pasted content (code, documents, data), it is wrapped in named tags and referenced by name in the instructions — see [authoring/PROMPT_STRUCTURE_GUIDE.md](authoring/PROMPT_STRUCTURE_GUIDE.md)
...
- [ ] **Example Output:** One coherent end-to-end scenario carried through every section of the stated Output Format — density, not line count
...
## Tier 1+ House-Style Patterns (analysis / evaluation / decision prompts)

These six moves recurred across a batch of high-scoring prompts and are the marks a prompt should hit when its job is to **analyze, evaluate, or decide** (as opposed to generate or transform).

1. **Two independent axes — never collapse the answer with confidence in the answer.** Report the substantive result and a separate meta-quality dimension, kept orthogonal (severity × confidence; rank × input-provenance; forward-case × reasoning-distortion). Corollary: *a conclusion can be right even when a stated reason is wrong* — flag the reasoning, keep the conclusion.
2. **Evidence-or-drop.** Every finding carries verifiable evidence *in the domain's own currency*, or it is downgraded/dropped — code review quotes the hunk + a failing input; concurrency shows a schedulable interleaving; a decision audit quotes the user verbatim; acceptance criteria carry a Verify-by; scoring carries a provenance tag. "This looks wrong" is a candidate, not a finding.
3. **Negative-space accounting.** Make the analysis's own limits visible so omissions are decisions, not accidents: a Question tier for findings that depend on unseen context; a Dismissed-candidates table with the guard that cleared each; an explicit Out-of-scope list; N/A-requires-a-reason on every swept-but-excluded category; provenance tags plus a coverage note.
4. **Mirror-image dual-failure (QA-20), done concretely.** Name **both** opposite harmful failures, and always include the *unhelpful* direction (hedging into mush, dumping options instead of choosing, refusing a deserved verdict, or moralizing/lecturing). State a memorable, testable bar.
5. **Decision-forcing output — with a first-class insufficiency verdict.** Terminate in a committed result derived by a *stated mechanical rule* (e.g. "REQUEST CHANGES iff ≥1 Blocking"). The rule MUST enumerate **INSUFFICIENT EVIDENCE** as its own verdict with a trigger and the single cheapest datum that unblocks it — so a forced verdict never fabricates a finding to fire. "INSUFFICIENT — pending Q1" is a committed verdict; "it depends" is banned.
6. **Loop-closure artifact.** End with a portable next step bound to a specific open item (a corrected decision statement, "next data to collect" tied to the costliest guess, a coverage note, or test-adequacy + risk notes). Generic next-steps are banned.

**Proportionality governs (move 5 wins).** At quick-fix scale, the negative-space and loop-closure instruments each compress to ≤1 line or are dropped, and the drop needs no justification. A safeguard must specify its **maximum** friction as explicitly as its minimum, or it rots into either bypassed or unbearable.

### Three standard-tightenings (all Tier-1 prompts)

1. **Coherent single-scenario Example Output** — one internally-consistent scenario whose findings reference each other, not disconnected fragments.
2. **Input provenance + a numeric self-recount for any computed number** — tag input provenance (`[data]`/`[estimate]`/`[guess]`), declare each numeric column's unit, and recompute ≥2 results from components.
3. **Anti-moralizing / anti-hedge constraint** — advice/audit/decision prompts carry an explicit *Must Not* against lecturing or caveating into uselessness: diagnose, quantify, correct, then commit. (Domain-required risk findings are *findings*, stated with severity and evidence — never "moralizing.")
