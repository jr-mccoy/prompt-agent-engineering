...
**Assessment Type:** [Quiz / Test / Performance Task / Project / Presentation / Portfolio]
**Grade Level:** [Grade]
...
Build assessment and rubrics through these systematic steps. **Steps are routed by assessment type** — complete only the steps that apply.
...
Determine the assessment type from the user's input, then create an alignment matrix using only the relevant item types:
...
**Assessment Type Routing — complete ONLY the steps that match the item types in your blueprint:**

| Assessment Type | Complete These Steps |
|-----------------|---------------------|
| **Quiz / Test (MC + SA)** | Steps 2, 3, 6, 7, 8, 9, 10 |
| **Essay / Extended Response** | Steps 4, 6, 7, 8, 9, 10 |
| **Performance Task / Project** | Steps 4, 6, 7, 8, 9, 10 |
| **Mixed (MC + SA + ER)** | Steps 2, 3, 4, 6, 7, 8, 9, 10 |

**Note:** For essay-only or single-task performance assessments, the prompt/task description AND rubric are combined in Step 4. There is no separate "Performance Task" step — Step 4 handles both.

### Step 2: Multiple Choice Items (Quiz/Test types only)

Skip this step if the assessment has no MC items.
...
### Step 3: Short Answer Items (Quiz/Test types only)

Skip this step if the assessment has no SA items.
...
### Step 4: Extended Response / Essay / Performance Task

Skip this step if the assessment has no ER or Performance items.

This step covers BOTH the prompt/task design AND the rubric. For assessments that are entirely essay- or performance-based, this is the primary assessment content step.

**4a. Prompt or Task Design**

For **essays/extended responses**, design the prompt:

**Essay Prompt Design Criteria:**
- The prompt must present an arguable or interpretable topic (not a factual recall question)
- The topic must be accessible to all students regardless of background, culture, or socioeconomic status
- The prompt must be grade-appropriate in vocabulary and complexity
- Requirements (length, structure, sources) must be explicit and countable
- The prompt should specify what students MAY and MAY NOT use (notes, internet, etc.)
...
### Step 5: (Reserved — no longer used)

---

### Step 6: Rubric Quality Check

Verify rubric meets quality standards:
...
### Step 7: Answer Key and Scoring Guide

Provide scoring guidance matched to the assessment type:
...
**For ER/Performance items:**
- Reference the rubric from Step 4b
...
### Step 9: Student-Facing Assessment

Create a clean, print-ready version of the assessment for students that includes:
- Assessment title, student name/date fields
- All prompts, questions, and response spaces
...
**Important:** The student version must NOT include answer keys, exemplary responses, misconception labels, or scoring descriptors.

### Step 10: Data Analysis Template

Create a post-assessment data collection template that includes:
- **Scoring Record Sheet:** Student names × criteria scores, totals, and accommodation flags
- **Performance Distribution Table:** Percentage of students at each level (Exemplary/Proficient/Developing/Beginning) per criterion, with mastery rate (Proficient + Exemplary)
- **Learning Objective Mastery Analysis:** Map criteria back to objectives, calculate class mastery rate per objective, flag objectives needing reteach
...
Structure the assessment package as:
1. **Assessment Blueprint** (Step 1 — objectives-to-items matrix with type routing)
2. **Assessment Items** (Steps 2-4 — only the item types that apply)
3. **Analytical Rubric** (Step 4b — with exemplary response sample)
4. **Rubric Quality Check** (Step 6 — all indicators must pass)
5. **Answer Key / Scoring Guide** (Step 7 — with calibration protocol for ER/Performance)
6. **Accommodations Guide** (Step 8 — specific accommodations, not generic)
7. **Student-Facing Assessment** (Step 9 — clean, print-ready, no answer keys)
8. **Data Analysis Template** (Step 10 — scoring records, performance distribution, action plan)
