...
│
├─→ User asks for a NEW prompt?
│   ("Create a new prompt for...", "Build me a prompt that...")
│   │
│   ├─→ Is it for IMAGE GENERATION?
│   │   (Badge buddy, infographic, worksheet image, diagram, logo, visualization)
│   │   │
│   │   └─→ BUILD FROM SCRATCH (Image Generation)
│   │       1. Read domain-image-generation/IMAGE_GENERATION_GUIDE.md
│   │       2. Apply the 8 core techniques:
│   │          - Terminology Steering (avoid "card", use "flat print artwork")
│   │          - Grid Forcing + Enumerated Slots (explicit NxM, BOX 1, BOX 2...)
│   │          - Constraint Redundancy (repeat "no gradients" 3+ times)
│   │          - Negative Space Control (ban backgrounds, shadows, mockups)
│   │          - Allowed vs Forbidden (structured layouts OK, UI looks forbidden)
│   │          - Physical Context Anchoring (real-world usage)
│   │          - Deliverables Locking (exact count, dimensions, orientation)
│   │          - Validation Checklist (final self-audit block)
│   │       3. Use template from guide for prompt type (print card, infographic, etc.)
│   │       4. Add model-specific notes (DALL-E, Midjourney, Stable Diffusion)
│   │
│   ├─→ Is it CODING/TECHNICAL focused?
│   │   │
│   │   └─→ BUILD FROM SCRATCH (Coding)
│   │       1. Read AI_AGENT_QUICK_START.md
│   │       2. Follow 5-step process
│   │       3. Use techniques/MASTER_TECHNIQUE_INDEX.md for techniques
│   │       4. Use techniques/USE_CASE_LOOKUP.md for patterns
│   │       5. Build custom prompt for user's exact needs
│   │
│   └─→ Is it NON-CODING focused?
│       (Education, writing, healthcare, research, personal development, business communication, professional fields)
│       │
│       └─→ BUILD FROM SCRATCH (Non-Coding)
│           1. Read NON_CODING_QUICK_START.md
│           2. Classify task type: CREATE, LEARN, DECIDE, COMMUNICATE, IMPROVE, SIMULATE
│           3. Identify domain: Education, Creative Writing, Healthcare, Research, Personal Development, Professional Communication, Specialized Fields
│           4. Apply appropriate Task Type Pattern
│           5. Use 5 Elements: Intent Clarity, Audience, Context, Output Specification, Quality Indicators
│           6. Add verification and false-positive prevention
│
├─→ User asks for a NEW SKILL?
│   ("Create a skill for...", "Build me a skill that...", "Make a reusable...")
│   │
│   └─→ BUILD SKILL
│       1. Read authoring/skill-patterns/README.md
│       2. Classify skill type (WORKFLOW, TOOL, DOMAIN, CREATION, ANALYSIS, INTEGRATION, META)
│       3. Use SKILL_USE_CASE_LOOKUP.md for pattern selection
│       4. Use SKILL_PATTERN_INDEX.md for implementation details
│       5. Validate with SKILL_QUALITY_RUBRIC.md
│
├─→ User needs HELP with a task?
│   ("Analyze my code...", "Help me with security...", "Review my...")
│   │
│   └─→ FIND EXISTING RESOURCE
│       1. Check domain-agentic-resources/skills/ for relevant skill
│       2. If no skill, map to domain directory (see Category Mapping below)
│       3. Select best matching resource
│       4. Execute against user's context
│
└─→ User asks ABOUT prompts, skills, or techniques?
    ("How do I...", "What's the best way to...", "Can you explain...")
    │
    └─→ REFERENCE DOCUMENTATION
        - Prompts: AI_AGENT_QUICK_START.md, techniques/MASTER_TECHNIQUE_INDEX.md
        - Image Generation: domain-image-generation/IMAGE_GENERATION_GUIDE.md
        - Skills: authoring/skill-patterns/README.md
...
When building **new prompts from scratch**, use these files in order:

### 0. Prompt Index (Discovery & Navigation)
**Files:** `PROMPT_INDEX.json`, `PROMPT_INDEX.md`, `PROMPT_INDEX_GUIDE.md`
- **When to use:** Finding existing prompts by keyword, technique, or domain
- **What it provides:** Machine-readable index of all ~1,800 prompts with metadata (path, domain, category, techniques, keywords, description)
- **Use for:** Discovering relevant prompts before building new ones, programmatic access to prompt catalog
- **Key features:**
  - JSON format for scripting and tools
  - Markdown format for human browsing
  - Search by technique codes (ST-01, RT-02, etc.)
  - Filter by domain, category, keywords, difficulty
  - 51% of prompts have structured frontmatter metadata
  - Cross-references via `related_prompts` field

**Quick Search Examples:**
```python
# Python: Find all security prompts
import json
with open('PROMPT_INDEX.json') as f:
    index = json.load(f)
security = [p for p in index['prompts'] if 'security' in p['keywords']]
```

```bash
# CLI: Find React prompts
grep -i "react" PROMPT_INDEX.md
```

**When NOT to use:** If you already know the exact prompt file path or are browsing a specific domain directory.

---

### 1. Quick Start - Coding (Primary Resource for Technical)
**File:** `AI_AGENT_QUICK_START.md`
- **When to use:** Building new coding/technical prompts
- **What it provides:** 5-step process, templates, common patterns for code analysis, debugging, architecture
- **Start here for:** All new coding/technical prompt creation

### 1b. Quick Start - Non-Coding (Primary Resource for Everything Else)
**File:** `NON_CODING_QUICK_START.md`
- **When to use:** Building new non-coding prompts (education, writing, healthcare, research, business communication, personal development, specialized fields)
- **What it provides:** 6 task type patterns (CREATE, LEARN, DECIDE, COMMUNICATE, IMPROVE, SIMULATE), 5 universal elements, domain-specific guidance
- **Start here for:** All new non-coding prompt creation
- **Key insight:** Non-coding prompts require explicit audience specification, quality criteria without tests, and false-positive prevention for subjective outputs

### 1c. Quick Start - Image Generation (Primary Resource for Visual Content)
**File:** `domain-image-generation/IMAGE_GENERATION_GUIDE.md`
- **When to use:** Building prompts that will be used with AI image generators (DALL-E, Midjourney, Stable Diffusion, ChatGPT image generation)
- **What it provides:** 8 core techniques for reliable image generation, templates, troubleshooting, model-specific notes
- **Start here for:** Badge buddies, infographics, worksheets, diagrams, logos, visualizations
- **Key insight:** Image generation prompts are fundamentally different - they require explicit geometry constraints, constraint redundancy, and print terminology to avoid UI mockup behaviors

**The 8 Core Techniques:**
| Technique | What It Does |
|-----------|--------------|
| Terminology Steering | Avoid "card" (triggers UI), use "flat print artwork" |
| Grid Forcing + Enumerated Slots | Explicit NxM grids with BOX 1, BOX 2, etc. |
| Constraint Redundancy | Repeat "no gradients" at 3+ levels |
| Negative Space Control | Ban backgrounds, shadows, mockup staging |
| Allowed vs Forbidden | Clear distinction for layouts vs UI looks |
| Physical Context Anchoring | Real-world usage constrains design |
| Deliverables Locking | Exact count, dimensions, orientation |
| Validation Checklist | Final self-audit block |

### 2. Use Case Patterns
**File:** `techniques/USE_CASE_LOOKUP.md`
- **When to use:** Finding right techniques for task type
- **What it provides:** Pre-built patterns by use case, recommended technique combinations
- **Use for:** Quick reference by task type (analysis, creation, teaching, etc.)

### 3. Technique Reference
**File:** `techniques/MASTER_TECHNIQUE_INDEX.md`
- **When to use:** Need details on specific techniques
- **What it provides:** Complete catalog of 327 active techniques across 18 categories
- **Use for:** Deep dive on how techniques work and combine

### 4. Prompt Quality Standards
**File:** `PROMPT_QUALITY_STANDARDS.md`
- **When to use:** Upgrading existing prompts or ensuring new prompts meet quality standards
- **What it provides:** Quality tier definitions, templates, False-Positive Prevention patterns, upgrade checklists
- **Use for:** Understanding what makes a Tier 1 (production-grade) prompt vs lower tiers
- **Key insight:** False-Positive Prevention section is the #1 quality differentiator

### 5. Repository Structure
**File:** `README.md`
- **When to use:** Understanding what's available in the repo
- **What it provides:** Complete overview of all categories, counts, and organization
- **Use for:** Navigation and high-level understanding

### 6. Coding Agent Personas
**File:** `domain-agentic-resources/personas/README.md`
- **When to use:** Multi-agent workflows or persistent agent identities
- **What it provides:** 52 agent personas, orchestration patterns, pipeline coordination
- **Use for:** Complex autonomous workflows

### 7. Agent Skills Authoring System (Design Guides)
**File:** `authoring/skill-patterns/README.md`
- **Role:** Authoring System - Design patterns and templates for CREATING new skills
- **When to use:** Creating reusable, modular capabilities for Claude Code
- **What it provides:** Complete skill authoring system with 41 patterns across 7 categories
- **Use for:** Building production-grade Agent Skills from scratch

**Key files in authoring/skill-patterns/:**
| File | Purpose |
|------|---------|
| `README.md` | Skill authoring system overview and entry point |
| `SKILL_PATTERN_INDEX.md` | 41 patterns for skill design |
| `SKILL_USE_CASE_LOOKUP.md` | Pattern selection by use case |
| `SKILL_QUALITY_RUBRIC.md` | 100-point quality scoring |
| `templates/GOLD_STANDARD_SKILL.md` | Annotated example skill |

**Skill Types:**
- **WORKFLOW** - Multi-step sequential processes
- **TOOL** - Technology/tool mastery (Helm, Terraform, K8s, etc.)
- **DOMAIN** - Domain expertise (security, performance, architecture)
- **CREATION** - Generate artifacts (docs, configs, code)
- **ANALYSIS** - Troubleshoot and diagnose issues
- **INTEGRATION** - External API/service connections
- **META** - Create other skills or capabilities

**Pattern Categories (41 total):**
- **Structural Patterns (SP-01 to SP-08):** Organization and architecture
- **Workflow Patterns (WP-01 to WP-05):** Process and execution
- **Metadata Patterns (MP-01 to MP-04):** Discoverability and loading
- **Integration Patterns (IP-01 to IP-05):** External systems
- **Resource Patterns (RP-01 to RP-06):** Bundled assets
- **Quality Patterns (QP-01 to QP-06):** Validation and safety
- **Meta-Generation Patterns (MG-01 to MG-02):** Skill creation

### 8. Coding Agents Implementation Library
**File:** `domain-agentic-resources/README.md`
- **Role:** Implementation Library - Production-ready agents, skills, and commands to USE
- **When to use:** Finding existing resources to use or study
- **What it provides:** 330 skills, 143 agents, 115 commands, 53 personas
- **Use for:** Executing existing capabilities or learning from examples

### Clarification: authoring/ vs domain-agentic-resources/

These two directories serve **complementary purposes**:

| Directory | Role | Contains | Use When |
|-----------|------|----------|----------|
| **`authoring/`** | Authoring System | Design patterns, templates, quality rubrics | **Creating** new resources |
| **`domain-agentic-resources/`** | Implementation Library | 330 skills, 143 agents, 115 commands, 53 personas | **Using** existing resources |
...
│   │
│   ├── Use a skill → domain-agentic-resources/skills/
│   ├── Use an agent → domain-agentic-resources/agents/
│   ├── Use a command → domain-agentic-resources/commands/
│   └── Use a persona → domain-agentic-resources/personas/
│
└─→ CREATE a new resource?
    ...
    └── Design an agentic system → authoring/system-patterns/README.md
        (or run the guided factory → agentic-system-factory/)
```

**Key Skill Categories:**
- **Cloud Infrastructure:** Helm charts, K8s manifests, Terraform, AWS, GCP
- **Developer Tools:** GitHub operations, Git workflows, skill creation
- **Testing & QA:** Test generation, E2E testing, performance testing
- **Security:** Security audits, vulnerability scanning, compliance
- **Documentation:** API docs, technical writing, diagram generation
- **DevOps:** CI/CD, Docker, monitoring, deployment
- **Database:** Schema design, migrations, optimization
- **Backend Development:** API design, microservices, event sourcing
- **Frontend Development:** React, Vue, accessibility, performance
- **Mobile Development:** iOS, Android, React Native, Flutter
- **Document Processing:** PDF creation, markdown processing

**Example Skills to Explore:**
- `helm-chart-scaffolding` - Generate production-ready Helm charts
- `github-ops` - Automated GitHub repository operations
- `skill-creator` - Meta-skill for creating new skills
- `pdf-creator` - Generate PDFs from markdown and templates
- `kubernetes-troubleshooting` - Diagnose K8s cluster issues
...
### DO:
- **Use existing prompts/skills** when user needs help with a task
- **Build prompts from scratch** when user explicitly asks for a new prompt
- **Build skills from scratch** when user wants a reusable capability
- **Customize existing resources** with user's specific context (tech stack, frameworks, etc.)
- **Combine multiple resources** for complex multi-faceted requests
- **Reference technique/pattern docs** when building new prompts or skills
- **Follow naming conventions** if user wants to save to repo

### DON'T:
- Build a new prompt/skill when an existing one would work (inefficient)
- Skip the repository search when user needs task help
- Ignore the 5-step process when building new prompts or skills
- Forget to customize existing resources for user's context
- Over-complicate prompts (3-5 techniques) or skills (focus on one thing)
- Create a skill when a simple prompt would suffice

---

## Naming Conventions

If user wants to save a prompt to the repository:
...
**Coding Agent Personas:**
```
{domain}_{role_descriptor}.md
Examples:
...
```
{domain}_{specific_function}.md
Examples:
- decisioning_blind_spot_identifier.md
- parenting_meltdown_response_script.md
- goals_skill_breakdown_blueprint.md
```
...
| "Is this even an ML problem?" | `domain-AI-ML/problem-framing-scoping/mlframe_is_this_an_ml_problem.md` |
...
| "We've had to remove a mentor — what do we say to everyone else?" | `domain-discipleship/after-harm/discipleship_after_a_mentor_is_removed.md` |
...
| "There's no program — we're just two people meeting" | `domain-discipleship/pairing-and-relationship/discipleship_informal_pairing_without_a_program.md` |
| "Someone's offered to disciple me and I don't know what's normal" | `domain-discipleship/pairing-and-relationship/discipleship_what_to_expect_as_a_mentee.md` |
...
| "We've multiplied three generations out and nobody knows what's being taught" | `domain-discipleship/curriculum-architecture/discipleship_multiplication_governance_and_material_drift.md` |
...
| "Frame a Scripture-rooted care conversation (NOT therapy)" | `domain-biblical-studies/ministry-contexts/biblical_ministry_biblical_care_conversation_foundations.md` |
...
| "Coordinate teaching across services/campuses" | `domain-biblical-studies/church-staff-ministry-ops/biblical_churchstaff_multi_service_teaching_coordination.md` |
...
| "Which sense fits / semantic domains (Louw-Nida)?" | `domain-biblical-studies/original-languages/biblical_language_semantic_domains_componential_analysis.md` |
...
| "Hebrew cantillation (te'amim) / Greek accentuation" | `domain-biblical-studies/original-languages/biblical_language_hebrew_accents_greek_accentuation_analysis.md` |
...
| "Koine register / papyri / inscriptions evidence" | `domain-biblical-studies/original-languages/biblical_language_koine_inscriptions_papyri_register_analysis.md` |
...
| "Grandparent / third-party custody or visitation" | `domain-legal/custody/legal_grandparent_third_party_custody_visitation_analysis.md` |
...
| "Document identity theft + prepare my reports" | `domain-legal/personal-self-advocacy/identity-theft/legalprep_identity_theft_documentation_and_report_preparer.md` |
...
| "How do we tell the kids we're separating?" | `domain-parenting/caregiver-facing/divorce/parenting_divorce_telling_the_kids_script.md` |
...
| "Am I the problem? Co-parenting self-audit" | `domain-parenting/caregiver-facing/co-parenting/parenting_coparenting_self_audit_am_i_the_problem.md` |
...
| "Self-designed exposure (with my therapist)" | `domain-psychology/client-self-use/coping-by-concern/clientself_anxiety_self_designed_exposure_with_therapist.md` |
...
| "OCD ERP draft (with my therapist)" | `domain-psychology/client-self-use/coping-by-concern/clientself_ocd_erp_self_designed_exercise_with_therapist.md` |
...
| "Anti-toxic-positivity journal prompts" | `domain-psychology/client-self-use/mood-journaling/clientself_journal_prompt_generator_anti_toxic_positivity.md` |
...
**Context:** Long sessions in this repo (4,400+ prompts, 293 active techniques, 318 skills) burn context fast. A prior web session timed out mid-task because Claude kept re-reading the same files and re-verifying facts it had already established. Auto-compact is configured to fire at 70% via `.claude/settings.json` — but the budget only holds if you stop wasting tokens.

**Apply these rules at all times, without being asked:**

### Do NOT re-read files mid-task
- If you have already `Read` a file in this session, **do not Read it again** unless (a) you or the user have edited it since, or (b) you only read a narrow offset/limit slice and now need a different part. Rely on what is already in your context.
- Before any `Read` call, ask yourself: "Have I already seen this file's contents in this conversation?" If yes, skip the read.
- Do not "double-check" file contents after an `Edit` succeeded — the Edit tool's success response is authoritative. Only re-read if the tool reported an error or you need to verify a follow-up change.
- Do not re-list directories you've already listed unless the filesystem has changed.

### Do NOT re-verify established facts
- Facts established earlier in the conversation (file paths, frontmatter fields, technique IDs, directory structure, repo conventions) remain valid. Do not re-run `Glob`, `Grep`, or `Bash ls` to confirm things you already confirmed.
- Do not re-run `git status` or `git branch` between closely-spaced operations unless state has meaningfully changed.
- Do not re-read `CLAUDE.md`, `PROMPT_INDEX.json`, `MASTER_TECHNIQUE_INDEX.md`, or other reference files more than once per session — extract what you need on first read and work from memory.
- Do not re-verify that a directory exists after you've already written a file into it successfully.

### Plan before reading
- Before opening any file, state (to yourself) what specific information you need from it. If you can answer the question without opening the file, don't open it.
- Prefer `Grep` over `Read` when you need a specific symbol, string, or section — ripgrep returns only the lines you need, not the whole file.
- When you do `Read`, use `offset` and `limit` for large files instead of reading the whole thing.

### Batch, don't iterate
- Run independent reads/searches in parallel (single message, multiple tool calls) instead of sequentially.
- Make edits in larger, complete chunks rather than many tiny sequential `Edit` calls to the same file.

### Trust tool results
- If `Write` or `Edit` returned success, the change is applied. Don't read the file back to "confirm."
- If `Bash` returned expected output, don't re-run the command to double-check.
- If a search returned no results, trust it — don't rerun with slight variations hoping for different output unless you have a specific reason to believe the pattern was wrong.

### Stop when done
- Once the user's task is complete, stop. Do not perform "just in case" verification passes, re-audits, or summary re-reads of files you just wrote.
- A concise confirmation is the end state, not a trigger for more work.

**Why this matters:** Every redundant `Read`, `Grep`, or `Bash` call consumes context that could be spent on actual work. The 70% auto-compact threshold assumes you aren't burning tokens on duplicative verification. Follow these rules and a single session can finish multi-file tasks that would otherwise time out.

---

**Last Updated:** 2026-06-20
**Agentic Authoring Update (2026-06-20):** Integrated the Agentic AI Authoring System — added routing for the `authoring/system-patterns/` manual, the `agentic-system-factory/` guided factory, and the 4 net-new `aiagent_*` prompts (complexity-ladder gate, hard-gates designer, orchestrator generator, agentic safety/eval layer).
