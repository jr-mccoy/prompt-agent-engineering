...
**Project Continuity Memory** — a portable, repo-local, human-readable ledger of
durable project state for human–agent software work.
...
- **Package / repo name:** `continuity-kit`
- **CLI binary name:** `continuity`
...
`continuity-kit` is a stdlib-only Python package (no third-party runtime
dependencies) that installs a single `continuity` binary. The recommended path
is [`pipx`](https://pipx.pypa.io/), which puts the CLI on your PATH in its own
isolated environment:

```bash
pipx install continuity-kit          # from a published artifact (future)
...
ships **inside the package** (`continuity_kit/templates/`), so `init` finds it
wherever the package lives — there is no repo-relative path dependency:

```bash
continuity --version                 # continuity-kit X.Y.Z (record schema_version N)
continuity init                      # locates bundled templates post-install
```

**Versioning.** The package uses semantic versioning. `continuity --version`
...
ships via `pipx`/`pip`. JavaScript-ecosystem reach (an `npx continuity` wrapper)
...
> **Two invocation forms.** Once installed (above), run `continuity <command>`.
> From a **source checkout** without installing, the equivalent is
> `python continuity.py <command>` (a thin shim over `continuity_kit.cli`); the
> per-command examples below use that source form. They are interchangeable.

```bash
continuity init                       # install .project-memory/ + manifest + .gitignore rules
continuity validate                   # deterministically check the store (schema + invariants)
continuity remember decision          # capture a durable choice
continuity capture session            # record session end (git-prefilled); updates handoff + current
continuity resume                     # print a bounded resume packet with computed staleness
continuity search "auth middleware"   # deterministic keyword/tag/file lookup over records
continuity guard "rewrite the auth middleware"   # warn before repeating a known mistake
continuity audit                      # heuristic health/safety report (stale/unsafe/bloated)
continuity scan-secrets               # block if committed memory holds token-like strings
```
...
```bash
python continuity.py init                                   # prompt for session policy (default: full)
python continuity.py init --session-tracking distillate     # keep sessions/ local
python continuity.py init --no-commit-generated             # keep generated/*.md local
python continuity.py init --project /path/to/repo --json    # init elsewhere, JSON summary
python continuity.py init --force                           # overwrite an existing scaffold
```
...
```bash
python continuity.py validate                      # human-readable report; exit 1 on problems
python continuity.py validate --json               # structured findings + exit code
python continuity.py validate --verbose            # also list the passing checks
python continuity.py validate --project /path/repo # validate elsewhere
```
...
### `continuity remember decision | attempt`

```bash
# non-interactive (agent-friendly): title + sections + evidence as flags
python continuity.py remember decision \
...
python continuity.py remember attempt --title "Tried a sqlite store" \
...
```bash
python continuity.py capture session --next "wire up the resume packet"   # git-prefilled
python continuity.py capture session --fast --next "tired — resume here"    # ~15s, no prompts
```
...
```bash
python continuity.py resume                       # full bounded packet (writes generated/resume-packet.md)
python continuity.py resume --fast                # git snapshot + focus + next action + staleness (print-only)
python continuity.py resume --json                # structured packet (sections + warnings) for agents
python continuity.py resume --stale-days 14       # tighten the aged-unresolved threshold (default 21)
```
...
```bash
python continuity.py search "auth middleware"        # keyword search over records
python continuity.py search --tag auth               # filter by tag/component
python continuity.py search --file src/auth/x.ts     # filter by referenced file path
python continuity.py search "session" --type decision --json
```
...
```bash
python continuity.py guard "rewrite the auth middleware"                 # human report (§11 shape)
python continuity.py guard "delete the accounts table" --files src/db/accounts.ts
python continuity.py guard "store the token in the url" --json           # structured, for agents
```
...
named `GUARD_*` thresholds at the top of the guard section in `continuity_kit/cli.py`, so it can
...
The tool degrades gracefully when `continuity` cannot run (e.g. a read-only cloud
...
| `pipx`/`pip` packaging (`continuity` console script, bundled templates) | implemented (Phase 7) |
| MCP server (`continuity-mcp`: 8 resources, 6 prompts, 7 tools) | implemented (Phase 8 — **optional**) |

With Phase 6 the full MVP (capture → resume → trust) is complete and CI-guarded;
Phase 7 packages it as a `pipx`-installable `continuity` binary (see **Install**
above). Phase 8 adds an **optional** MCP server (`pip install
"continuity-kit[mcp]"`) that exposes the same memory engine to agents without
