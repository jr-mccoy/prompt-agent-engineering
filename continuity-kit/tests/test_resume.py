"""Tests for `continuity resume` and the resume packet (Phase 4 — MVP-core / 19a).
...
def copy_fixture(dest_parent: str) -> Path:
    ...
    shutil.copytree(FIXTURE / ".project-memory", dest / continuity.MEMORY_DIRNAME)
    return dest
...
class FixtureSixQuestionsTests(unittest.TestCase):
    ...
    def test_fixture_validates(self):
        with tempfile.TemporaryDirectory() as tmp:
            mem = copy_fixture(tmp) / continuity.MEMORY_DIRNAME
...
class SourceHeaderTests(unittest.TestCase):
    def test_packet_carries_source_header(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(continuity.GENERATED_MARKER, out)
...
class FastModeTests(unittest.TestCase):
    ...
    def test_fast_does_not_overwrite_committed_packet(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = copy_fixture(tmp)
            mem = root / continuity.MEMORY_DIRNAME
...
class TokenBoundTests(unittest.TestCase):
    def test_many_records_stay_within_budget(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(code, 0)
            payload = json.loads(out)
            self.assertLessEqual(payload["approx_tokens"], continuity.TOKEN_BUDGET_MAX)
            # Per-section cap + budget trim must have kicked in.
            self.assertLessEqual(len(payload["active_decisions"]), continuity.SECTION_CAPS["active_decisions"])
