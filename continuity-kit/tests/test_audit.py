"""Tests for `continuity audit` (Phase 6, plan §10/§15/§16 note/§19b).
...
def run(argv: list[str]) -> tuple[int, str]:
    ...
    with contextlib.redirect_stdout(buf):
        code = continuity.main(argv)
    return code, buf.getvalue()


def fresh_store(tmp: str) -> Path:
    continuity.main(["init", "--project", tmp, "--session-tracking", "full"])
    return Path(tmp) / continuity.MEMORY_DIRNAME
...
class HealthViewTests(unittest.TestCase):
    ...
    def test_branch_mismatch_surfaced(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            findings = continuity.run_audit(mem, root)
...
class InstructionLikeTests(unittest.TestCase):
    def test_audit_flags_override_phrasing(self):
        ...
        self.assertTrue(all(f["severity"] == continuity.AUDIT_WARN for f in il))
    ...
    def test_validate_does_not_flag_instruction_like(self):
        ...
        fails = [f for f in continuity.run_validate(mem) if f["status"] == "fail"]
...
class PacketDriftTests(unittest.TestCase):
    def test_audit_flags_stale_packet(self):
        ...
        self.assertEqual(drift[0]["severity"], continuity.AUDIT_WARN)
    ...
    def test_drift_detect_unit(self):
        ...
        drift = continuity.detect_packet_drift(mem)
...
class BloatTests(unittest.TestCase):
    def test_sessions_growth_note(self):
        ...
        self.assertEqual(bloat[0]["severity"], continuity.AUDIT_INFO)

    def test_adapter_duplication_flagged(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            mem = fresh_store(tmp)
            rec_path, _ = continuity.write_record(
