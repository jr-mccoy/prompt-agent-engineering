    ...
    def test_all_fixtures_validate_clean(self):
        for name in ALL_FIXTURES:
            with self.subTest(fixture=name):
                fails = [f for f in continuity.run_validate(mem_of(name)) if f["status"] == "fail"]
...
class ManySessionsTests(unittest.TestCase):
    ...
    def test_resume_packet_stays_bounded_and_prioritised(self):
        ...
        packet = continuity.build_resume_packet(mem, root)
        md = continuity.render_packet_markdown(packet)
        self.assertLessEqual(continuity.approx_tokens(md), continuity.TOKEN_BUDGET_MAX)
