...
class ParserTests(unittest.TestCase):
    def test_roundtrips_full_schema(self):
        meta, body = continuity.parse_frontmatter(FULL)
    ...
    def test_no_frontmatter_returns_body_verbatim(self):
        text = "# Just a heading\n\nno frontmatter here\n"
        meta, body = continuity.parse_frontmatter(text)
        self.assertEqual(meta, {})
        self.assertEqual(body, text)

    def test_unterminated_fence_is_malformed(self):
        with self.assertRaises(continuity.FrontmatterError):
            continuity.parse_frontmatter("---\nkey: val\nno closing fence\n")
    ...
    def test_quoted_value_with_hash_is_preserved(self):
        meta, _ = continuity.parse_frontmatter('---\nref: "#42"\n---\n')
...
class RecordModelTests(unittest.TestCase):
    def test_sections_split_on_h2(self):
        _, body = continuity.parse_frontmatter(FULL)
        rec = continuity.Record(Path("decisions/2026-06-25-x.md"), "decision", {}, body)
    ...
    def test_from_file_captures_parse_error(self):
        ...
        rec = continuity.Record.from_file(bad, "decision")
    ...
    def test_load_records_walks_type_dirs(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            recs = continuity.load_records(mem)
            types = sorted(r.rtype for r in recs)
            self.assertEqual(types, ["decision", "session"])
            # type filtering
            only_dec = continuity.load_records(mem, types=("decision",))
