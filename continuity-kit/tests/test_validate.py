"""Tests for `continuity validate` (Phase 2, plan §16.1-14).
...
def fresh_store(tmp: str) -> Path:
    ...
    root = Path(tmp)
    continuity.main(["init", "--project", str(root), "--session-tracking", "full"])
    return root / continuity.MEMORY_DIRNAME
...
class ValidatePassTests(unittest.TestCase):
    def test_fresh_init_validates_clean(self):
        with tempfile.TemporaryDirectory() as tmp:
            mem = fresh_store(tmp)
            findings = continuity.run_validate(mem)
...
class ValidateFailTests(unittest.TestCase):
    ...
    def _run_with(self, subdir: str, fixture: str) -> set[str]:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            return checks_failing(continuity.run_validate(mem))
    ...
    def test_16_1_manifest_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("manifest", checks_failing(continuity.run_validate(mem)))

    def test_16_2_core_file_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            mem = fresh_store(tmp)
            (mem / "handoff.md").unlink()
            failing = checks_failing(continuity.run_validate(mem))
    ...
    def test_16_11_handoff_missing_sections(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("handoff", checks_failing(continuity.run_validate(mem)))

    def test_16_12_generated_without_marker(self):
        with tempfile.TemporaryDirectory() as tmp:
            mem = fresh_store(tmp)
            (mem / "generated" / "resume-packet.md").write_text("# looks canonical\n")
            self.assertIn("generated", checks_failing(continuity.run_validate(mem)))
...
class ValidateCliTests(unittest.TestCase):
    def test_json_exit_code_on_failure(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with contextlib.redirect_stdout(buf):
                code = continuity.main(["validate", "--project", tmp, "--json"])
    ...
    def test_json_exit_code_on_success(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            buf = io.StringIO()
            with contextlib.redirect_stdout(buf):
                code = continuity.main(["validate", "--project", tmp_path, "--json"])
            self.assertEqual(code, 0)
            self.assertTrue(json.loads(buf.getvalue())["ok"])

    def test_missing_store_errors(self):
        with tempfile.TemporaryDirectory() as tmp:
            code = continuity.main(["validate", "--project", tmp])
