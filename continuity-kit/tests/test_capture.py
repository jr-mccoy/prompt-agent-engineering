"""Tests for `continuity capture session` (Phase 3).
...
def init_store(root: Path, tracking: str = "full") -> Path:
    continuity.main(["init", "--project", str(root), "--session-tracking", tracking])
...
class CapturePrefillTests(unittest.TestCase):
    def test_prefills_work_and_files_from_git(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _, body = continuity.parse_frontmatter(path.read_text())
            rec = continuity.Record(path, "session", {}, body)
