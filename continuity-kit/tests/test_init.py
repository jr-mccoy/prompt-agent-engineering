"""Tests for `continuity init` (Phase 1).
...
# Make continuity.py importable regardless of cwd.
...
class GitignorePolicyTests(unittest.TestCase):
    ...
    def test_gitignore_block_is_idempotent(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(gi.count(continuity.GITIGNORE_BEGIN), 1)
            self.assertEqual(gi.count(continuity.GITIGNORE_END), 1)
