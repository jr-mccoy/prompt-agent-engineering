"""Tests for `continuity guard` — guard-before-action (Phase 5, plan §11 / §17).
...
def copy_fixture(name: str, dest_parent: str) -> Path:
    ...
    shutil.copytree(FIXTURES / name / ".project-memory", dest / continuity.MEMORY_DIRNAME)
    return dest
...
class Fixture2TruePositiveTests(unittest.TestCase):
    ...
    def test_match_carries_required_scoring_signals(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertGreater(att["score"], continuity.GUARD_NOISE_FLOOR)
...
class BoundTests(unittest.TestCase):
    def test_warnings_bounded_to_five(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertLessEqual(len(res["matches"]), continuity.GUARD_MAX_WARNINGS)
...
class JsonShapeTests(unittest.TestCase):
    def test_json_payload_keys(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(res["verdict"], continuity._VERDICTS)
            self.assertEqual(res["thresholds"]["max_warnings"], continuity.GUARD_MAX_WARNINGS)
