"""Tests for `continuity remember decision|attempt` (Phase 3).
...
class RememberDecisionTests(unittest.TestCase):
    ...
    def test_filename_id_agreement(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            meta, _ = continuity.parse_frontmatter(path.read_text())
            rid, slug = continuity.derive_identity(path.stem, "decision")
...
class RememberAttemptTests(unittest.TestCase):
    def test_attempt_creates_valid_record(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual([f for f in continuity.run_validate(mem) if f["status"] == "fail"], [])
...
class RememberMisuseTests(unittest.TestCase):
    ...
    def test_bare_remember_errors(self):
        with tempfile.TemporaryDirectory() as tmp:
            init_store(tmp)
            code = continuity.main(["remember", "--project", tmp])
            self.assertEqual(code, 2)
