...
class IdentityTests(unittest.TestCase):
    def test_decision_prefix(self):
        self.assertEqual(
            continuity.derive_identity("2026-06-25-repo-local-memory", "decision"),
    ...
    def test_all_type_prefixes(self):
        ...
        for rtype, prefix in cases.items():
            rid, slug = continuity.derive_identity("2026-01-02-thing", rtype)
            self.assertEqual(rid, f"{prefix}_20260102_thing")
            self.assertEqual(slug, "thing")

    def test_slug_keeps_internal_hyphens(self):
        rid, slug = continuity.derive_identity("2026-12-31-a-b-c", "attempt")
        self.assertEqual(slug, "a-b-c")
        self.assertEqual(rid, "att_20261231_a-b-c")

    def test_non_canonical_filename_returns_none(self):
        self.assertIsNone(continuity.derive_identity("not-a-dated-file", "decision"))
        self.assertIsNone(continuity.derive_identity("2026-6-5-bad-date", "decision"))
