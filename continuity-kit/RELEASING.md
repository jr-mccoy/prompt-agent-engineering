# Releasing continuity-kit to PyPI
...
pipx install --index-url https://test.pypi.org/simple/ \
             --pip-args="--extra-index-url https://pypi.org/simple/" continuity-kit
continuity --version
```

(The extra index lets pip resolve any real deps; continuity-kit itself has none.)
...
1. Create a token at **https://pypi.org/manage/account/token/** (scope it to the
   `continuity-kit` project after the first upload; before that, an account-wide
...
`continuity --version` prints both. Bump the package MAJOR only alongside a
