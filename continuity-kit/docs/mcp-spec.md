...
> [`continuity_kit/mcp_server.py`](../continuity_kit/mcp_server.py), a thin binding
> over the adapter core in [`continuity_kit/mcp_core.py`](../continuity_kit/mcp_core.py).
> Every resource/prompt/tool wraps the **same** Phase 1–6 functions the CLI calls
> ([`continuity_kit/cli.py`](../continuity_kit/cli.py)) — one source of behavior,
> no fork.
...
```bash
pip install "continuity-kit[mcp]"     # adds the `mcp` SDK (needs Python >=3.10)
```

Run the server (stdio transport):

```bash
continuity-mcp                         # console script
python -m continuity_kit.mcp_server    # equivalent module form
```

**Root resolution.** The server operates on the project in `$CONTINUITY_PROJECT`
...
`continuity_kit.mcp_server` still succeeds; `build_server()` raises a clear
install hint and `continuity-mcp` prints that hint and exits non-zero. Nothing
...
| `memory://resume-packet` | rendered packet markdown (identical to `continuity resume`) | `build_resume_packet` + `render_packet_markdown` |
...
|---|---|---|
| `resume_project` | `continuity resume` | orient from the resume packet before acting |
| `capture_session` | `continuity capture session` | wind a session down into durable memory |
| `remember_decision` | `continuity remember decision` | record a durable decision (evidence-backed) |
| `remember_attempt` | `continuity remember attempt` | record a failed attempt + "do not retry" |
| `guard_before_action` | `continuity guard` | check memory before a risky action |
| `audit_project_memory` | `continuity audit` | validate + secret-scan health check |
...
```jsonc
{
  "mcpServers": {
    "continuity-kit": {
      ...
      "env": { "CONTINUITY_PROJECT": "${workspaceFolder}" }
    }
  }
}
```

Equivalently `python -m continuity_kit.mcp_server`. The server requires the
