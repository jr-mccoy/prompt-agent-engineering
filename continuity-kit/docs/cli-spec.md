...
The CLI binary is `continuity` (installed via `pipx`/`pip` as of Phase 7; from a
source checkout the equivalent is `python continuity.py <command>`, a shim over
`continuity_kit.cli`). The command surface is stable from Phase 1: every command supports the
...
```bash
continuity init
continuity init --session-tracking <full|distillate>
continuity init --no-commit-generated
continuity init --project <path>
continuity init --force
```

Behavior:

- Refuses to clobber an existing `.project-memory/` unless `--force`.
- Copies the bundled `continuity_kit/templates/project-memory/` tree (shipped as
...
```bash
continuity resume                  # full bounded packet; writes generated/resume-packet.md
continuity resume --fast           # reduced reorientation view (print-only)
continuity resume --json           # structured packet (sections + warnings + source header)
continuity resume --stale-days N   # aged-unresolved threshold in days (default: 21)
```
...
```bash
continuity audit                  # human health report
continuity audit --json           # structured findings (check/severity/path/message)
continuity audit --plain          # one line per finding
continuity audit --stale-days N   # aged-unresolved threshold (default: 21)
```
...
```bash
continuity scan-secrets           # human report; non-zero on any hit
continuity scan-secrets --json    # {ok, count, hits:[{pattern, path, line}]}
```
