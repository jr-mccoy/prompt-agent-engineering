"""continuity-kit — MCP adapter core (Phase 8).
...
search/guard/resume/validate/audit/record all live in `continuity_kit.cli`.
...
from continuity_kit import cli
...
def _require_memory(memory_dir: Path) -> None:
    if not memory_dir.is_dir():
        raise FileNotFoundError(
            ...
            "Run `continuity init` first (or point at a project that has memory)."
        )
...
def _read_singleton(memory_dir: Path, name: str) -> str:
    ...
    p = memory_dir / name
    if not p.is_file():
        return f"_(no {name} — run `continuity init`)_"
...
def resource_resume_packet(root: str | Path | None = None) -> str:
    """`memory://resume-packet` — the rendered packet (same as `continuity resume`)."""
...
def prompt_resume_project(root: str | Path | None = None) -> str:
    return _prompt(
        """
You are resuming work on a software project that uses continuity-kit memory.
...
def prompt_capture_session(root: str | Path | None = None) -> str:
    return _prompt(
        """
Wind down this work session into durable memory (mirrors `continuity capture
...
def prompt_remember_decision(root: str | Path | None = None) -> str:
    return _prompt(
        """
Record a durable DECISION (mirrors `continuity remember decision`). Provide a
...
def prompt_remember_attempt(root: str | Path | None = None) -> str:
    return _prompt(
        """
Record a failed ATTEMPT so it is not repeated (mirrors `continuity remember
...
def prompt_audit_project_memory(root: str | Path | None = None) -> str:
    return _prompt(
        """
Audit the health and safety of project memory (mirrors `continuity audit`).
