"""continuity-kit — MCP server (Phase 8, Python SDK).
...
core functions in :mod:`continuity_kit.cli` (via :mod:`continuity_kit.mcp_core`):
...
Install the optional runtime with:  ``pip install "continuity-kit[mcp]"``
Run it with:                         ``python -m continuity_kit.mcp_server``
                                or:  ``continuity-mcp``

Root resolution: the server operates on the project in ``$CONTINUITY_PROJECT`` if
...
from continuity_kit import mcp_core
...
SERVER_NAME = "continuity-kit"
_INSTALL_HINT = (
    "The MCP server needs the optional Python MCP SDK.\n"
    '  pip install "continuity-kit[mcp]"   (or:  pip install mcp)\n'
    "Everything still works without it via the `continuity` CLI and the plain\n"
...
def _root() -> str | None:
    """Project root for this server process: $CONTINUITY_PROJECT or cwd."""
    return os.environ.get("CONTINUITY_PROJECT") or None
...
def build_server():  # -> FastMCP
    ...
    def memory_search(query: str, filters: dict | None = None) -> dict:
        """Deterministic search over canonical records (wraps `continuity search`)."""
    ...
    def memory_guard_before_action(action: str, files: list[str] | None = None) -> dict:
        """Guard-before-action; returns the same verdict as `continuity guard`."""
    ...
    def memory_build_resume_packet(task: str | None = None) -> dict:
        """Build the structured resume packet (wraps `continuity resume`)."""
        return mcp_core.tool_build_resume_packet(task=task, root=_root())

    @mcp.tool()
    def memory_validate() -> dict:
        """Run deterministic structural validation (wraps `continuity validate`)."""
    ...
    def memory_scan_secrets() -> dict:
        """Scan committed memory for secret-like strings (wraps `continuity audit`'s scan)."""
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for `python -m continuity_kit.mcp_server` / `continuity-mcp`."""
    if FastMCP is None:
