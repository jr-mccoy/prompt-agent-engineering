#!/usr/bin/env python3
"""continuity — Project Continuity Memory CLI.
...
Phase 7 (packaging): this is the package entry point (``continuity_kit.cli``),
exposed as the ``continuity`` console script. The ``continuity.py`` shim at the
repo root re-exports it so source-checkout use (``python continuity.py ...``)
and the test suite keep working unchanged. Templates ship as package data under
``continuity_kit/templates/`` so ``init`` finds them post-install without any
repo-relative path.
...
# `continuity_kit` package, so this resolves correctly both from a source
...
# Markers delimiting the block continuity manages inside the project .gitignore.
# Anything between them is rewritten by `init`; everything else is preserved.
GITIGNORE_BEGIN = "# >>> continuity-kit managed block (managed by `continuity init`) >>>"
GITIGNORE_END = "# <<< continuity-kit managed block <<<"
...
def manifest_content(
    ...
    return (
        ...
        f"project: {project}\n"
        f"# Tracking policy chosen during `continuity init` (see docs/record-schema.md):\n"
...
def write_gitignore(root: Path, block: str) -> None:
    """Insert/replace the continuity-managed block in the project .gitignore.
...
def _emit_init_summary(args: argparse.Namespace, summary: dict) -> None:
    ...
    if args.verbose:
        print("\nNext: `continuity remember decision` / `continuity capture session` (planned).")
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    if not memory_dir.is_dir():
        _emit_error(
            args,
            f"no {MEMORY_DIRNAME}/ found at {root}. Run `continuity init` first.",
        )
        return 2
...
def cmd_remember(args: argparse.Namespace) -> int:
    ...
    if rtype not in ("decision", "attempt"):
        _emit_error(args, "specify a record type: `continuity remember decision|attempt`")
        return 2

    root = resolve_root(args.project)
    memory_dir = root / MEMORY_DIRNAME
    if not memory_dir.is_dir():
        _emit_error(args, f"no {MEMORY_DIRNAME}/ found at {root}. Run `continuity init` first.")
        return 2
...
def cmd_capture_session(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        if tracking == "distillate":
            ...
            print("        promote durable items with `continuity remember` to commit them.")
    return 0
...
def render_packet_markdown(packet: dict) -> str:
    ...
    proj = packet["project"]
    out: list[str] = [
        f"<!-- {GENERATED_MARKER} — do not edit by hand. Rebuilt by `continuity resume`. -->",
        ...
        "",
        "## Next Action",
        packet["next_action"] or "_(not recorded — set one with `continuity capture session --next`)_",
        "",
    ]
    ...
    for d in detect_packet_drift(memory_dir):
        findings.append(
            _audit_finding(
                ...
                f"current {d['current']}) — regenerate with `continuity resume`",
                stamped=d["stamped"],
                current=d["current"],
            )
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="continuity",
        description="Project Continuity Memory — a repo-local ledger of durable project state.",
...
def _capture_dispatch(args: argparse.Namespace) -> int:
    """`continuity capture` with no subcommand -> guidance."""
    if getattr(args, "capture_what", None) is None:
        _emit_error(args, "specify what to capture: `continuity capture session`")
        return 2
