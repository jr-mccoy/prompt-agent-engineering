...
updated: "2026-07-23"
related_prompts:
  - domain-personal-development/prompts/productivity/productivity_energy_by_task_type.md
...
1. **The focus block.** When and where the user's deep-work window happens (time, place, device). One line. If they have no protected block at all, stop and route to `productivity_personal_energy_audit.md` / `domain-productivity/deep-work/` — a ritual needs a block to trigger into.
...
1. **Don't design a ritual when the block itself is broken.** If input 2 shows the block is constantly interrupted or never protected, the problem is the calendar, not the onset. Route to `productivity_personal_energy_audit.md` / `domain-productivity/deep-work/` first.
...
5. **Don't confuse onset-freeze with the wrong energy window.** If the user can't start because the task is jammed into an energy trough, fix the timing first via `productivity_energy_by_task_type.md`; a ritual can't beat a biology mismatch.
