...
- **PAE Registry** — the governed resource corpus that exists today, plus the normalized machine-readable metadata layer being built on top of it.
- **PAE Engine** — a planned installable package (`pae-engine/`) for search, routing, context compilation, validation, evaluation, and MCP access.

The Engine is **planned and not yet implemented**. Everything described below as available today is documentation, corpus, and validation tooling. See [`ARCHITECTURE.md`](ARCHITECTURE.md) for the current-versus-planned boundary and [`ROADMAP.md`](ROADMAP.md) for sequencing.
