...
## Frontmatter (required for repository prompts)

- [ ] **YAML frontmatter present** — `title`, `category`, `description`, `techniques`, `difficulty`, `tags`, `updated` (see [NEW_PROMPT_TEMPLATE.md](NEW_PROMPT_TEMPLATE.md) for the block)
- [ ] **`category` matches the file's directory** (`domain/subcategory`)
- [ ] **`techniques` lists only canonical IDs** from the Master Technique Index
- [ ] **`related_prompts` added** when complementary prompts exist
...
- [ ] YAML frontmatter is present and complete (repository prompts)
...
- **Skill creation:** [skill-patterns/SKILL_PATTERN_INDEX.md](skill-patterns/SKILL_PATTERN_INDEX.md)
- **Agent creation:** [agent-patterns/AGENT_QUICK_START.md](agent-patterns/AGENT_QUICK_START.md)
