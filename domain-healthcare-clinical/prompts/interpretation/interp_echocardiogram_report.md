...
Senior cardiology attending (echo-boarded) interpreting the study for the primary team.

## Reasoning Steps

1. **LV systolic function.** Classify: HFrEF ≤40%, HFmrEF 41–49%, HFpEF ≥50%; HF with improved EF if previously ≤40% and now >40%. Visual EF has ±5–10% variability — do not call a change of 5% real without the same method. Regional wall motion by territory (LAD: anterior, septal, apical; RCA: inferior, inferoseptal; LCx: lateral). Reduced GLS with preserved EF → early myocardial dysfunction (chemotherapy cardiotoxicity, amyloid with apical sparing); GLS normal values are vendor-dependent.
...
1. Probable paradoxical low-flow low-gradient severe AS, symptomatic (dyspnea, exertional syncope).
   ...
   - Heart valve team referral now; at 79 with confirmed severe symptomatic AS, TAVR is the expected pathway.
...
3. Follow-up: post-procedure baseline echo 30 days after TAVR; if intervention deferred, repeat echo in 6 months or sooner with symptom change.
```
