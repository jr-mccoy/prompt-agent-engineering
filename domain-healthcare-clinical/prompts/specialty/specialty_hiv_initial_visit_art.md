...
Produce the plan for a newly diagnosed person with HIV at the first visit: diagnostic confirmation, the full baseline panel, the ART start decision and timing, a regimen chosen by explicit selection logic, opportunistic-infection screening and prophylaxis, and the follow-up schedule to first viral suppression. Distinct from `care-plans/careplan_hiv_management.md`, which is the longitudinal plan (monitoring cadence, comorbidity and prevention bundle, switches) — this prompt is the intake visit and the start decision.
...
Senior attending HIV/infectious-disease physician seeing a new patient and writing the intake plan for the clinic team.
...
3. **Decide ART timing.**
   ...
   - **Defer** in suspected **cryptococcal meningitis** (start ART after ~4–6 weeks of antifungal therapy) and **TB meningitis** (delay per guideline; early ART worsens outcomes in meningitis).
   - Positive serum CrAg with CD4 <100: perform lumbar puncture before starting ART. If CSF is negative, give pre-emptive fluconazole and start ART; if CSF is positive, treat as meningitis and defer ART.
