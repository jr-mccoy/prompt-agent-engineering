---
title: "Local SMB Image Advertising Prompt Builder"
category: advertising/image-generation
description: "Interview-driven meta-prompt for generating print-safe and screen-safe advertising image prompts for local smb."
techniques:
  - SV-03
  - ST-02
  - SV-11
...
**Objective:** Generate a high-compliance advertising image prompt for **Local SMB** campaigns using an interview-first workflow and strict print/screen output constraints.

## Interview Intake (required before prompt generation)

Collect and confirm all fields before drafting the final image prompt:
1. **Product/Offer:** `local shop promotion, seasonal SMB offer`
2. **Audience:** `nearby residents and repeat customers`
3. **Key Message:** `Trusted local value`
4. **Palette Direction:** `brand colors + neighborhood cues`
5. **Primary CTA:** `Visit today`
...
- ROLE + purpose
- campaign context for local smb
...
- **SV-16 Physical Context Anchoring:** include realistic usage context for local smb advertising collateral.
