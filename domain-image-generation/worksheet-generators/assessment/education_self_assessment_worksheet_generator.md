...
**Purpose:** Generate learner self-assessment worksheets with rubric-style reflection, evidence prompts, and action-step planning.
...
```text
LAYOUT ZONES
- ZONE 1: Header (title, learning target, name/date).
- ZONE 2: Directions for honest self-rating and evidence use.
- ZONE 3: Criteria table with rating scale (e.g., 1-4 or emerging→mastery).
- ZONE 4: Evidence/examples response box.
- ZONE 5: Strengths + growth area prompts.
- ZONE 6: Next-step goal and teacher/family signature line.

CONTENT LOCK
- Use provided grade level to choose age-appropriate reflection language.
- Use provided topic/standard for all rubric criteria.
- Embed required vocabulary in criteria labels and reflection stems.
- Keep rating symbols text-based (no color-only distinctions).
```
