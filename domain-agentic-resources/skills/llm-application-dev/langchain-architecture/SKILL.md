...
To validate your LangChain application is production-ready:

1. **Agent Reliability**: Agent completes target task >= 90% of the time on test scenarios
2. **Memory Management**: Conversation history stays within token limits after 20+ turns
3. **Tool Selection**: Agent selects correct tool >= 95% of the time on labeled test set
4. **Error Handling**: All tool failures and LLM errors are caught and produce graceful fallbacks
5. **Latency**: End-to-end response time meets SLA (typically < 10s for agent chains)
6. **Cost**: Token usage per request is within budget at projected volume
7. **Observability**: Callbacks log all chain steps for debugging and monitoring

## Related Skills

- `langchain-optimization` - Performance, cost, and reliability optimization for LangChain apps (use after establishing a working application)
- `rag-architecture` - RAG pipeline design for document retrieval chains
- `embedding-strategies` - Embedding model selection for LangChain vector stores
- `vector-search-optimization` - Vector database tuning for LangChain retrievers
