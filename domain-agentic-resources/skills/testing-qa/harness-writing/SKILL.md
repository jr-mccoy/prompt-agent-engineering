...
Tool-specific harness signatures, compilation commands, integration tips, and running instructions for libFuzzer (LLVMFuzzerTestOneInput + FuzzedDataProvider.h + clang++ -fsanitize=fuzzer,address), AFL++ (persistent mode with __AFL_LOOP + __AFL_INIT + afl-clang-fast++), cargo-fuzz (fuzz_target! macro + arbitrary crate + cargo fuzz init/add/run), and go-fuzz (Fuzz(data []byte) int + return codes + go-fuzz-build), plus a troubleshooting table (7 rows), related skills table (tools + techniques), and external resources.

See [references/tool-specific-guidance.md](references/tool-specific-guidance.md)
...
| Resource | Purpose |
|----------|---------|
| `references/tool-specific-guidance.md` | Tool-specific harness signatures and compilation for libFuzzer, AFL++, cargo-fuzz, go-fuzz, plus troubleshooting, related skills, and resources |
