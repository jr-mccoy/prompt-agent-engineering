...
Fuzzing Python C Extensions (environment configuration, cbor2 example), Sanitizer Integration (ASan, LD_PRELOAD, common issues table), Advanced Usage (tips/tricks, custom instrumentation, performance tuning, UBSan), and Real-World Examples (JSON parser, HTTP request parsing) are in the reference file.

See [references/c-extensions-sanitizers-examples.md](references/c-extensions-sanitizers-examples.md)
...
| Resource | Purpose |
|----------|---------|
| `references/c-extensions-sanitizers-examples.md` | Fuzzing Python C Extensions (env config + cbor2 example), Sanitizer Integration (ASan/LD_PRELOAD/common issues), Advanced Usage (tips/custom instrumentation/performance tuning/UBSan), Real-World Examples (JSON parser + HTTP request parsing) |
