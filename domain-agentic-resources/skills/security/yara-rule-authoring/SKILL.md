...
## Decision Trees & Expert Heuristics

Seven decision trees for string quality assessment, all/any usage, rule abandonment criteria, false positive debugging, string type selection (text/hex/regex), packed sample detection, and structure-based pivots when strings fail; plus expert heuristics covering string selection priorities, condition ordering, quality signals, modifier discipline (nocase/wide costs), regex anchoring requirements, loop bounding, YARA-X tips, and module vs byte-check selection guidance.

See [references/decision-trees-and-heuristics.md](references/decision-trees-and-heuristics.md)
...
| Decision trees and expert heuristics | [decision-trees-and-heuristics.md](references/decision-trees-and-heuristics.md) |
