...
## DFD Analysis, STRIDE per Interaction, Best Practices & Resources

DFD analysis (DFDAnalyzer with trust boundary crossing detection ranked by trust-level difference; element-type-to-threat-category mapping for EXTERNAL_ENTITY/PROCESS/DATA_STORE/DATA_FLOW; unencrypted flow detection across trust boundaries; generate_threat_enumeration over all elements), StridePerInteraction with INTERACTION_THREATS dict covering external→process (all 6 STRIDE), process→datastore (T/R/I/D), and process→process (S/T/I/D/E) interaction types; generate_threat_matrix iterating all Interaction objects with element-type lookup; best practices (Do's/Don'ts), resources (Microsoft STRIDE docs/OWASP/Shostack).

See [references/dfd-analysis-and-stride-per-interaction.md](references/dfd-analysis-and-stride-per-interaction.md)
...
| Resource | Purpose |
|----------|---------|
| `references/dfd-analysis-and-stride-per-interaction.md` | DFD analysis, STRIDE per interaction, best practices, resources |
