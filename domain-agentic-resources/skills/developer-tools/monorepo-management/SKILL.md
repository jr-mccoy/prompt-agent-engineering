...
## Build Optimization, CI/CD, and Publishing

Turborepo caching configuration (inputs/outputs/dependsOn; remote cache with Vercel or custom), GitHub Actions CI workflow (pnpm setup + frozen-lockfile install + turbo run build/test/lint/type-check), deploying only affected apps (nx affected:apps with grep filtering), best practices (versioning/shared configs/acyclic deps/cache inputs/type safety/testing strategy/documentation/release strategy), common pitfalls (circular deps/phantom deps/incorrect cache inputs/over-sharing/under-sharing/large repos), and publishing with Changesets (pnpm changeset init/version/publish + release.yml GitHub Actions).

See [references/build-optimization-and-cicd.md](references/build-optimization-and-cicd.md)
...
| Resource | Purpose |
|----------|---------|
| `references/build-optimization-and-cicd.md` | Turborepo caching, remote cache, GitHub Actions CI, deploy affected, best practices, common pitfalls, Changesets publishing |
