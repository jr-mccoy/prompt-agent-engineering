...
Risk management as a working discipline: identify risks, structure them, prioritize them, harden against them, and learn when one materializes anyway. The twenty-four prompts here adapt the standard instruments — appetite statement, register, heat map, FMEA, bow-tie, threat model, dependency audit, third-party assessment, key risk indicators, Monte Carlo quantification, acceptance memo, board report, tail-risk scan, continuity plan, after-action review — for general operators, not just safety engineers, and add a security-operations set for organisations without a SOC (incident playbook, tabletop exercise, alert triage, phishing awareness, vendor security review, payment-fraud controls), and each enforces the discipline that makes its instrument worth more than a spreadsheet: registers get named owners and review cadences, heat maps force the "are we actually doing the top-3 mitigations?" question, AARs run a blameless root-cause ladder instead of finding a culprit.

The domain spans the full lifecycle. Before the fact: build the catalogue (`risk_register_builder`), hunt what the catalogue systematically misses (`risk_tail_risk_scan`), model intelligent adversaries (`risk_threat_model_non_technical`), decompose a process's failure modes (`risk_fmea_analysis`), and find the single points of failure (`risk_dependency_chain_audit`). Quantify and watch: put ranges on the top risks (`risk_monte_carlo_quantification`), map the barriers around one major hazard (`risk_bow_tie_analysis`), assess each critical outside party as a whole (`risk_third_party_risk_assessment`), and set indicators that move before the loss (`risk_key_risk_indicators`). Govern: record deliberate acceptances with an expiry (`risk_acceptance_exception_memo`), visualize and re-rank (`risk_heat_map`), and report upward (`risk_board_risk_report`). After: learn without scapegoating (`risk_after_action_review`).
...
- **Whole-vendor selection on fit and cost** → `domain-business-strategy/research/research_vendor_evaluation.md` or `domain-operations/supply-chain-procurement/ops_supplier_selection_scorecard.md`; this domain assesses the risk an already-chosen (or shortlisted) vendor carries — `risk_third_party_risk_assessment.md` for the whole vendor, `risk_vendor_security_questionnaire_review.md` for its security answers.
- **Market VaR on a trading book, or counterparty credit exposure** → `domain-finance/risk-management/`; `risk_monte_carlo_quantification.md` is for operational, project and enterprise loss ranges.
- **Full operational-risk control self-assessment (RCSA)** → `domain-finance/risk-management/finance_operational_risk_rcsa.md`; `risk_key_risk_indicators.md` designs the indicators themselves.
- **The board's finance package or its slides** → `domain-finance/corporate-finance-fpa/finance_board_finance_package_builder.md`, `domain-presentations/powerpoint_board_deck.md`; `risk_board_risk_report.md` is the risk paper.
...
| `risk_third_party_risk_assessment.md` | Whole-vendor risk across six dimensions (financial, operational, concentration, compliance, security, fourth parties and exit); worst dimension sets the rating; conditions, owner, re-assessment triggers |
| `risk_key_risk_indicators.md` | Leading and lagging KRIs with formula, source, green/amber/red thresholds anchored to appetite or history, owner action per band, back-test against past incidents, gaming vectors |
| `risk_bow_tie_analysis.md` | One hazard: threats → top event → consequences, preventive and mitigative barriers rated effective/degraded/missing on evidence, degradation factors, independence check, thin paths ranked |
| `risk_monte_carlo_quantification.md` | Calibrated ranges, distributions and shared-driver correlation simulated in named software; loss exceedance curve against appetite, sensitivity, plain-frequency summary, no fake precision |
| `risk_acceptance_exception_memo.md` | Formal acceptance or policy exception: scope, inherent/residual rating, compensating controls with checks, alternatives costed, approver matched to authority, hard expiry, revisit triggers |
| `risk_board_risk_report.md` | Periodic board or committee risk paper: headlines, decisions requested, top-risk movement, appetite breaches, non-green KRIs, expiring acceptances, incidents, emerging risks |
...
| "Which risks should get attention this cycle?" / "draw our risk posture" | `risk_heat_map.md` |
| "We need the quarterly risk paper for the board / risk committee" | `risk_board_risk_report.md` |
| "How would we know this risk is getting worse?" / "our monitoring column says 'watch closely'" | `risk_key_risk_indicators.md` |
| "How much could we lose in a bad year?" / "is this mitigation worth its cost?" | `risk_monte_carlo_quantification.md` |
| "We say we have controls for this one big hazard — do we?" | `risk_bow_tie_analysis.md` |
| "Can we keep relying on this vendor / outsourcer?" / "it changed owners" | `risk_third_party_risk_assessment.md` |
| "We can't fix it yet — who signs off on living with it, and until when?" | `risk_acceptance_exception_memo.md` |
...
The backbone chain is **register → heat map → review cadence**: `risk_register_builder` produces the scored catalogue, `risk_heat_map` visualizes and re-ranks it each cycle, and when a register risk triggers anyway, `risk_after_action_review` closes the loop and feeds revisions back into the register. `risk_tail_risk_scan` is the register's adversary — run it after the register exists to find what the register's frame excluded, then log the survivors. The two decomposition tools slot in by subject shape: `risk_fmea_analysis` when the subject has discrete steps or components, `risk_dependency_chain_audit` when the question is concentration and single points of failure (often downstream of `systems_dependency_map.md`, which draws the wiring this prompt ranks). `risk_threat_model_non_technical` is the special case for moves with an intelligent adversary; its outputs also feed the register as adversarial risk entries. `risk_bow_tie_analysis` takes one register hazard a level deeper, into barriers and their evidence.

The governance chain runs **register → indicators and quantification → acceptance → board report**. `risk_key_risk_indicators` gives each top register risk a measure with thresholds anchored in `risk_appetite_statement`; `risk_monte_carlo_quantification` puts loss ranges on the same top risks and reads the exceedance curve against the same appetite. Where a risk stays above appetite, `risk_acceptance_exception_memo` records who accepted it and until when, and its measurable revisit triggers become KRIs. `risk_board_risk_report` assembles movement, breaches, non-green KRIs, expiring acceptances and incidents for the board, with `risk_heat_map` as its appendix visual. `risk_third_party_risk_assessment` sits beside the dependency audit: it takes replaceability from `risk_dependency_chain_audit`, the security dimension from `risk_vendor_security_questionnaire_review`, and hands kept-but-High vendors to the acceptance memo.
